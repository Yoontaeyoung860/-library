...
export default class BookService {
  public static async getBooks(token: string): Promise<BookResType[]> {
    const response: AxiosResponse<BookResType[]> = await axios.get<BookResType[]>(BOOK_API_URL, {
      headers: {
  ...
  public static async addBook(
    ...
    const response: AxiosResponse<BookResType> = await axios.post<BookResType>(BOOK_API_URL, book, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
  ...
  public static async editBook(
    ...
    const response: AxiosResponse<BookResType> = await axios.patch<BookResType>(
      `${BOOK_API_URL}/${bookId}`,
      book,
      {
        headers: {
